...
Releases are cut from the changelog. To cut `vX.Y.Z`:

1. Bump `version` to `vX.Y.Z`.
2. Run `go generate ./...` so the committed codecs carry the new header.
3. Add a `## [vX.Y.Z] - YYYY-MM-DD` section to the top of
   [`CHANGELOG.md`](CHANGELOG.md).
4. Merge to `main`.

The [`changelog-release`](.github/workflows/changelog-release.yml) workflow then
parses that entry, verifies its version matches the `version` constant and that
generated code is up to date, and creates the tag and GitHub release with the
changelog section as the notes. It is a no-op unless the top entry is a new,
versioned, dated section whose tag does not yet exist, so the Git tag and the
generated-file version can never disagree.
